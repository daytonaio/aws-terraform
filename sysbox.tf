resource "kubectl_manifest" "sysbox_service_account" {
  yaml_body = <<YAML
apiVersion: v1
kind: ServiceAccount
metadata:
...
resource "kubectl_manifest" "sysbox_cluster_role_binding" {
  yaml_body = <<YAML
...
resource "kubectl_manifest" "sysbox_daemonset" {
  yaml_body = <<YAML
apiVersion: apps/v1
kind: DaemonSet
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: sysbox-deploy-k8s
        image: registry.nestybox.com/nestybox/sysbox-deploy-k8s:v0.6.1
...
resource "kubectl_manifest" "sysbox_runtime_class" {
  yaml_body = <<YAML
